no overall control description supplied
* Building
no zone control description supplied
   4  # No. of functions
* Control function    1
# senses the temperature of the current zone.
    0    0    0    0  # sensor data
# actuates within Base-6 in bedroom.
    1    6   17  # actuator data
    1  # all daytypes
    1  365  # valid Mon-01-Jan - Mon-31-Dec
     5  # No. of periods in day: weekdays    
    0    1   0.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  1168.000 0.000 1000.000 0.000 18.000 24.000 0.000
    0    1   6.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  1168.000 0.000 1000.000 0.000 20.000 24.000 0.000
    0    1   8.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  1168.000 0.000 1000.000 0.000 18.000 24.000 0.000
    0    1  18.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  1168.000 0.000 1000.000 0.000 20.000 24.000 0.000
    0    1  21.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  1168.000 0.000 1000.000 0.000 20.000 24.000 0.000
* Control function    2
# senses the temperature of the current zone.
    0    0    0    0  # sensor data
# actuates within Base-12 in livingroom.
    2   12   17  # actuator data
    1  # all daytypes
    1  365  # valid Mon-01-Jan - Mon-31-Dec
     5  # No. of periods in day: weekdays    
    0    1   0.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  2600.000 0.000 1000.000 0.000 18.000 24.000 0.000
    0    1   6.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  2600.000 0.000 1000.000 0.000 20.000 24.000 0.000
    0    1   8.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  2600.000 0.000 1000.000 0.000 18.000 24.000 0.000
    0    1  18.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  2600.000 0.000 1000.000 0.000 20.000 24.000 0.000
    0    1  21.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  2600.000 0.000 1000.000 0.000 20.000 24.000 0.000
* Control function    3
# senses the temperature of the current zone.
    0    0    0    0  # sensor data
# actuates mixed convective/radiant flux in bathroom.
   -2    3   30  # actuator data
    1  # all daytypes
    1  365  # valid Mon-01-Jan - Mon-31-Dec
     5  # No. of periods in day: weekdays    
    0    1   0.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  1500.000 0.000 1000.000 0.000 18.000 24.000 0.000
    0    1   6.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  1500.000 0.000 1000.000 0.000 20.000 24.000 0.000
    0    1   8.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  1500.000 0.000 1000.000 0.000 18.000 24.000 0.000
    0    1  18.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  1500.000 0.000 1000.000 0.000 20.000 24.000 0.000
    0    1  21.000  # ctl type, law (basic control), start @
      7.  # No. of data items
  1500.000 0.000 1000.000 0.000 20.000 24.000 0.000
* Control function    4
# senses the temperature of the current zone.
    0    0    0    0  # sensor data
# actuates air point of the current zone
    0    0    0  # actuator data
    1  # all daytypes
    1  365  # valid Mon-01-Jan - Mon-31-Dec
     1  # No. of periods in day: weekdays    
    0    2   0.000  # ctl type, law (free floating), start @
      0.  # No. of data items
# Function:Zone links
 1,2,3,4
